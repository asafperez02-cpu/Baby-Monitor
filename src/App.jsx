    ...
    <div style={S.mainWidget}>
      ...
      <div style={S.nextFeedBox} onClick={onOpenFutureFeeds}>
        <div style={{display:'flex', alignItems: 'center', justifyContent: 'space-between'}}>
          ...
          <div style={{background: C.peach, color: 'white', borderRadius: '12px', padding: '6px 14px', fontSize: 13, fontWeight: 800}}>
            תחזית
          </div>
        </div>
      </div>
...
// ── AI Component (Hugging Face Version) ──────────────────────────────────
function AiModal({ events, onClose }) {
  ...
  const saveKey = () => {
    if(localKey.trim().length > 10) {
      localStorage.setItem("hf_key", localKey.trim());
  ...
  const askAi = async () => {
    ...
    setAns("מנתחת נתונים... ✨");
    try {
      const history = events.slice(0, 10).map(e => {
      ...
      const response = await fetch("https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.2", {
        ...
        body: JSON.stringify({ inputs: `<s>[INST] הנה נתוני התינוקת עלמה: ${history}. שאלה: ${q}. ענה בעברית קצרה מאוד. [/INST]` })
      });

      const data = await response.json();
      const rawText = data[0]?.generated_text || "";
      // ניקוי התשובה מסימני ה-Inst
      const cleanedAns = rawText.split('[/INST]').pop().trim();
      setAns(cleanedAns || "לא הצלחתי לקבל תשובה.");
  ...
  return (
    <div style={S.overlay} onClick={onClose}><div style={S.modal} onClick={e=>e.stopPropagation()}>
        <h3 className="kids-font" style={{textAlign:'center', color:C.peachDark}}>העוזרת של עלמה (HF) ✨</h3>
        {isEditingKey ? (
          <div>
            <p style={{fontSize:12, textAlign:'center', color:C.textSoft, marginBottom: 10}}>הדבק מפתח API (hf_...) מ-Hugging Face:</p>
      ...
      <div style={S.card}>
        ...
        <div style={{ position: 'relative', width: '100%', height: 160 }}>
          <svg viewBox="0 0 320 160" style={{ width: '100%', height: '100%' }}>
            ...
            {points.map((p, i) => (
              <g key={i}><circle cx={p.x} cy={p.y} r="5" fill="white" stroke={C.peachDark} strokeWidth="3" /><text x={p.x} y={p.y - 12} textAnchor="middle" fontSize="12" fontWeight="800" fill={C.text}>{p.ml}</text></g>
            ))}
          </svg>
        </div>
      </div>
...
const S = {
  ...
  headerContainer: { background: `linear-gradient(135deg, ${C.peach}, #f9a8d4)`, padding: "calc(15px + env(safe-area-inset-top)) 20px 25px", borderRadius: "0 0 45px 45px", textAlign: "center", boxShadow: "0 8px 25px rgba(232, 121, 249, 0.3)" },
  ...
  nextFeedBox: { marginTop: 15, background: "rgba(255,255,255,0.7)", padding: "12px 16px", borderRadius: "18px" },
