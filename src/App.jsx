  ...
  return (
    <div style={S.app}>
      <style>{`
        ...
        * { box-sizing: border-box; font-family: ${FONT_MAIN}; }
      ...
      <div style={S.headerContainer}>
        ...
        <div style={{fontSize: 38, color: "white", fontWeight: 800, marginBottom: 15, fontFamily: FONT_KIDS}}>עלמה 🌸</div>
...
function MainTimerWidget({ events, now, onOpenFutureFeeds }) {
  ...
  return (
    <div style={S.mainWidget}>
      <div style={{fontSize: 14, color: 'white', opacity: 0.9}}>אכלה לפני:</div>
      <div style={{fontSize: 48, fontWeight: 900, color: 'white', fontFamily: FONT_KIDS}}>🍼 {timeStr}</div>
...
function AiModal({ events, onClose }) {
  ...
  const askAi = async () => {
    ...
    try {
      const history = events.slice(0, 15).map(e => `${new Date(e.ts).getHours()}:${new Date(e.ts).getMinutes()}: ${e.type === 'feed' ? `אכלה ${e.ml}ml` : 'חיתול'}`).join(', ');
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${localKey.trim()}`, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ text: `נתונים: ${history}. שאלה: ${q}. ענה בעברית קצרה.` }] }] })
      });
      const data = await res.json();
      setAns(data.candidates?.[0]?.content?.parts?.[0]?.text || "שגיאה בנתונים.");
  ...
  return (
    <div style={S.overlay} onClick={onClose}><div style={S.modal} onClick={e=>e.stopPropagation()}>
      <h3 style={{textAlign:'center', color:C.peachDark}}>העוזרת של עלמה ✨</h3>
      {isEditingKey ? (
        ...
        <>
          <input placeholder="שאל אותי משהו..." value={q} onChange={e=>setQ(e.target.value)} style={S.input} onKeyDown={e=>e.key==='Enter'&&askAi()} />
          <button onClick={askAi} disabled={loading} style={S.primaryBtn}>{loading?"...":"שאל אותי"}</button>
          {ans && <div style={S.aiResponse}>{ans}</div>}
          <button onClick={()=>setIsEditingKey(true)} style={{background:'none', border:'none', color:C.textSoft, fontSize:11, marginTop:15, textDecoration:'underline', width:'100%'}}>עדכון מפתח</button>
        </>
      )}
    </div></div>
  );
...
function FutureFeedsModal({ events, onClose }) {
  ...
  return (
    <div style={S.overlay} onClick={onClose}><div style={S.modal} onClick={e=>e.stopPropagation()}>
      <h3 style={{textAlign:'center', color:C.peachDark}}>תחזית ארוחות ⏰</h3>
...
function FeedModal({ onConfirm, onClose }) {
  ...
  return (
    <div style={S.overlay} onClick={onClose}><div style={S.modal} onClick={e=>e.stopPropagation()}>
      <h3 style={{textAlign:'center', marginBottom: 20}}>האכלה 🍼</h3>
...
function DiaperModal({ onConfirm, onClose }) {
  return (
    <div style={S.overlay} onClick={onClose}><div style={S.modal} onClick={e=>e.stopPropagation()}>
      <h3 style={{textAlign:'center', marginBottom: 20}}>החתלה 🧷</h3>
