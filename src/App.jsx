...
export default function BabyApp() {
  ...
  // ── Notification: 10 min before 4h since last feed ──
  useEffect(() => {
    if (!("Notification" in window)) return;
    const lastFeed = events.find(e => e.type === "feed");
    if (!lastFeed) return;
    const ALERT_MS = 3 * 60 * 60 * 1000 + 50 * 60 * 1000;
    const sinceLastFeed = Date.now() - lastFeed.ts;
    const timeToAlert = ALERT_MS - sinceLastFeed;
    if (timeToAlert <= 0) return;
    const tid = setTimeout(async () => {
      if (Notification.permission === "granted") {
        new Notification("🍼 זמן האכלה מתקרב!", { body: "עוד 10 דקות יגיעו 4 שעות מהארוחה האחרונה" });
...
// ── Feed Status Hero ───────────────────────────────────────────────────────
function FeedStatusHero({ lastFeed, lastDiaper, now }) {
  const sinceMs = lastFeed ? now - lastFeed.ts : null;
  const sinceMin = sinceMs ? Math.floor(sinceMs / 60000) : null;
  const hours = sinceMin !== null ? Math.floor(sinceMin / 60) : null;
  const mins = sinceMin !== null ? sinceMin % 60 : null;

  // Color shifts from green -> yellow -> red as time passes
  let bgGrad, ringColor, textColor;
  if (sinceMin === null) {
    bgGrad = `linear-gradient(135deg, #fde8f2, #fdeee8)`;
    ringColor = "#f9a8c9";
    textColor = "#e879a0";
  } else if (sinceMin < 120) {
    bgGrad = `linear-gradient(135deg, #d1fae5, #a7f3d0)`;
    ringColor = "#34d399";
    textColor = "#065f46";
  } else if (sinceMin < 210) {
    bgGrad = `linear-gradient(135deg, #fef9c3, #fde68a)`;
    ringColor = "#f59e0b";
    textColor = "#92400e";
  } else {
    bgGrad = `linear-gradient(135deg, #ffe4e6, #fca5a5)`;
    ringColor = "#f87171";
    textColor = "#991b1b";
  }

  const timeStr = sinceMin === null
    ? "עוד לא אכלה"
    : hours > 0
  ...
  return (
    <div style={{
      ...
      textAlign: "center",
      boxShadow: `0 6px 24px ${ringColor}40`,
      ...
      <div style={{
        ...
        border: `3px solid ${ringColor}20`,
        pointerEvents: "none",
      }} />

      <div style={{ fontSize: 13, color: textColor, fontWeight: 700, opacity: 0.8, marginBottom: 4 }}>
        🍼 אכלה לפני
      </div>
      <div style={{
        fontSize: 52,
      ...
      {lastFeed && (
        <div style={{ fontSize: 12, color: textColor, opacity: 0.7, marginTop: 6 }}>
          בשעה {new Date(lastFeed.ts).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" })}
          {lastFeed.ml ? ` · ${lastFeed.ml} מ"ל` : ""}
        </div>
      )}

      {/* Divider */}
      <div style={{ borderTop: `1px solid ${ringColor}30`, margin: "14px 0 10px" }} />

      {/* Diaper sub-line */}
      <div style={{ fontSize: 13, color: textColor, opacity: 0.75, fontWeight: 600 }}>
        🧷 {lastDiaper
          ? `הוחלפה לפני ${(() => { const m = Math.floor((Date.now() - lastDiaper.ts) / 60000); return m < 60 ? m + " דק'" : Math.floor(m/60) + "ש' " + (m%60) + "ד'"; })()}`
          : "טרם תועדה החתלה"}
      </div>
    </div>
  );
...
function HomeTab({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* ── Feed Status Hero ── */}
      <FeedStatusHero lastFeed={lastFeed} lastDiaper={lastDiaper} now={now} />
